# -*- coding: utf-8 -*-
"""
Created on Mon Jul 13 14:28:30 2020
...
"""
# Data_frame数据，5s作为输入
# 3 layer CNN: original data, (1467,489), Test accuracy: 0.893.  添加BN层，Test accuracy: 0.9345
# 3 layer CNN: data_aug 4 times, (7335,489), Test accuracy: 0.8875,  添加BN层，Test accuracy: 0.92638

# 8 layer VGGish: data_aug, Test accuracy: 0.9775
...
def extract_features(path, file_ext="*.wav"):
    ...
    for i in range(1956):
        ...
        melspec = librosa.feature.melspectrogram(sound_clip, n_mels = 128, fmax=8000)
        logspec = librosa.power_to_db(melspec)
        logspec_1 = logspec.T.flatten()[0:128*213, np.newaxis]
        log_specgrams.append(logspec_1)
            
        labels.append(label)       
        print('The file of number %s has done' %i)
    features = np.asarray(log_specgrams).reshape(len(log_specgrams), melspec.shape[0], 213, order="F")
...
def data_augment(data, label):    
    ...
    for i in range(X_train.shape[0]):
        ...
        labels.append(label[i])
        print('The file of number %s has done' %i)
    features = np.asarray(X_train_aug).reshape(len(X_train_aug), 128, 213, order="F")
    return np.array(features), np.array(labels,dtype = np.int)
...
# 5s数据，提取所有数据的 Mel 特征     
    if not os.path.isfile('D:/Project/Sound/ShipsEar/Feature model/data_feature_5s.npy'): 
        data_features,labels = extract_features(path)
        data_labels = to_categorical(labels)
        np.save('data_feature_5s',data_features)
        np.save('data_label_5s', data_labels)
    else:
        data_features = np.load('D:/Project/Sound/ShipsEar/Feature model/data_feature_5s.npy')
        data_labels = np.load('D:/Project/Sound/ShipsEar/Feature model/data_label_5s.npy')        
...
# # 对训练集进行数据增广
# =============================================================================
# if not os.path.isfile('D:/Project/Sound/ShipsEar/Feature model/data_feature_aug_5s.npy'):
#     train_aug_features, train_aug_labels = data_augment(X_train, Y_train)
#     train_aug_labels = to_categorical(train_aug_labels)
#     np.save('data_feature_aug_5s', train_aug_features)
#     np.save('data_label_aug_5s', train_aug_labels)
# else:
#     train_aug_features = np.load('D:/Project/Sound/ShipsEar/Feature model/data_feature_aug_5s.npy')
#     train_aug_labels = np.load('D:/Project/Sound/ShipsEar/Feature model/data_label_aug_5s.npy')   
...
X_train = X_train.reshape(X_train.shape[0], 128, 213, 1)
X_test = X_test.reshape(X_test.shape[0], 128, 213, 1)
